...
class DocumentService:
    ...
    def update_category(self, category_id: str, data: Dict[str, Any]) -> Optional[DocumentCategory]:
    ...
    def record_view(self, document_id: str, user_id: str = None) -> None:
        ...
        if user_id:
            if user_id not in self.analytics['user_activity']:
                self.analytics['user_activity'][user_id] = {'views': {}, 'downloads': {}}
    ...
    def get_dashboard_stats(self) -> Dict[str, Any]:
        ...
        for category in categories:
            ...
            category_stats[category.name] = {
                ...
                'views': sum(self.analytics['views'].get(d.id, 0) for d in category_docs),
                'downloads': sum(self.analytics['downloads'].get(d.id, 0) for d in category_docs)
            }
        ...
        for doc_type in DocumentType:
            ...
            type_stats[doc_type.value] = {
                ...
                'downloads': sum(self.analytics['downloads'].get(d.id, 0) for d in type_docs)
            }
    ...
    def _generate_daily_report(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        for i in range(days):
            ...
            if date in self.analytics['daily_stats']:
                daily_data.append({
                    'date': date,
                    **self.analytics['daily_stats'][date]
                })
            else:
                daily_data.append({
                    'date': date,
    ...
    def _generate_category_report(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        for category in categories:
            ...
            total_views = sum(self.analytics['views'].get(d.id, 0) for d in category_docs)
            total_downloads = sum(self.analytics['downloads'].get(d.id, 0) for d in category_docs)
            ...
            category_data.append({
                ...
                'total_views': total_views,
                'total_downloads': total_downloads,
                'avg_views_per_doc': total_views / len(category_docs) if category_docs else 0,
                'avg_downloads_per_doc': total_downloads / len(category_docs) if category_docs else 0
            })
    ...
    def _generate_user_report(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        user_data.sort(key=lambda x: x['total_views'] + x['total_downloads'], reverse=True)
    ...
    def _generate_popular_report(self, filters: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        for doc in documents:
            ...
            if views > 0 or downloads > 0:
                popular_data.append({
                    'document_id': doc.id,
                    'title': doc.title,
                    'category': self.get_category(doc.category_id).name if self.get_category(doc.category_id) else 'N/A',
                    'type': doc.type,
                    'status': doc.status,
                    'views': views,
                    'downloads': downloads,
