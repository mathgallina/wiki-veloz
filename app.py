    ...
    try:
        ...
        for activity in activities:
            if activity['action'] == 'page_view':
                page_title = activity['details'].split(': ')[-1] if ': ' in activity['details'] else 'Página'
        ...
        # Últimas atividades
        latest_activities = sorted(activities, key=lambda x: x['timestamp'], reverse=True)[:10]
        ...
        return jsonify({
            ...
            'top_pages': [{'title': title, 'views': views} for title, views in top_pages],
...
def get_page_analytics():
    ...
    for page in pages:
        ...
        size_category = 'Pequena' if content_length < 1000 else 'Média' if content_length < 5000 else 'Grande'
...
def get_user_analytics():
    ...
    for activity in activities:
        ...
        if not user_activity[user_id]['last_activity'] or activity['timestamp'] > user_activity[user_id]['last_activity']:
