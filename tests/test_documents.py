"""
Testes unitários para o módulo de documentos corporativos
"""
import json
import os
import tempfile
...
class TestDocumentService:
    ...
    def setup_method(self):
        ...
        self.temp_docs_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json')
        self.temp_docs_file.write('[]')
        self.temp_docs_file.close()
        
        self.temp_categories_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json')
        self.temp_categories_file.write('[]')
        self.temp_categories_file.close()
        
        self.temp_versions_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json')
        self.temp_versions_file.write('[]')
        self.temp_versions_file.close()
        
        self.temp_analytics_file = tempfile.NamedTemporaryFile(mode='w', delete=False, suffix='.json')
        self.temp_analytics_file.write('{"views": {}, "downloads": {}, "daily_stats": {}, "category_stats": {}, "type_stats": {}, "user_activity": {}}')
        self.temp_analytics_file.close()
        
        # Mock dos repositórios
        with patch('app.modules.documents.services.DocumentRepository') as mock_repo:
            with patch('app.modules.documents.services.DocumentCategoryRepository') as mock_cat_repo:
                with patch('app.modules.documents.services.DocumentVersionRepository') as mock_ver_repo:
                    self.service = DocumentService()
                    self.service.repository.data_file = self.temp_docs_file.name
                    self.service.category_repository.data_file = self.temp_categories_file.name
                    self.service.version_repository.data_file = self.temp_versions_file.name
                    self.service.analytics_file = self.temp_analytics_file.name
    
    def teardown_method(self):
        """Cleanup após cada teste"""
        for file_path in [self.temp_docs_file.name, self.temp_categories_file.name, 
                         self.temp_versions_file.name, self.temp_analytics_file.name]:
    ...
    def test_create_document(self):
        ...
        with patch.object(self.service.repository, 'create') as mock_create:
            with patch.object(self.service.version_repository, 'create') as mock_ver_create:
    ...
    def test_get_dashboard_stats(self):
        ...
        mock_docs = [
            Document(id='doc-1', title='Doc 1', category_id='cat-1', type='ata', status='ativo'),
            Document(id='doc-2', title='Doc 2', category_id='cat-1', type='regulamento', status='ativo')
        ]
        
        mock_categories = [
            DocumentCategory(id='cat-1', name='Test Category')
        ]
        
        with patch.object(self.service, 'get_all_documents', return_value=mock_docs):
            with patch.object(self.service, 'get_all_categories', return_value=mock_categories):
                # Mock dos analytics
                self.service.analytics = {
...
class TestDocumentAPI:
    ...
    def test_get_documents_api(self):
        ...
        with patch('app.modules.documents.routes.documents_service.get_all_documents') as mock_get:
    ...
    def test_create_document_api(self):
        ...
        with patch('app.modules.documents.routes.documents_service.create_document') as mock_create:
            ...
            with patch('app.modules.documents.routes.validator.validate_document_data') as mock_validate:
                mock_validate.return_value = {'valid': True}
                
                response = self.client.post('/documents/api/documents',
                                         json={
                                             'title': 'Test Document',
                                             'description': 'Test description',
                                             'category_id': 'cat-1',
    ...
    def test_create_document_api_validation_error(self):
        """Testa API de criação com erro de validação"""
        with patch('app.modules.documents.routes.validator.validate_document_data') as mock_validate:
    ...
    def test_get_document_analytics_api(self):
        ...
        with patch('app.modules.documents.routes.documents_service.get_document_analytics') as mock_analytics:
    ...
    def test_record_view_api(self):
        ...
        with patch('app.modules.documents.routes.documents_service.record_view') as mock_record:
    ...
    def test_record_download_api(self):
        ...
        with patch('app.modules.documents.routes.documents_service.record_download') as mock_record:
